...
class CrugeAuthassignment extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('userid, activo', 'numerical', 'integerOnly'=>true),
			array('itemname', 'length', 'max'=>64),
			array('bizrule, data, fecha', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('userid, itemname, bizrule, data, activo, fecha', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'tAPs' => array(self::MANY_MANY, 'TAP', 'Evaluador(Usuario_id, TAP_id)'),
		);
	}
